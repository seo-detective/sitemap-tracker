...
# --- HEADERS: THE "SPLIT PERSONALITY" ---
# 1. Standard Chrome (Works for Forbes, NYT, Bloomberg)
HEADERS_CHROME = {
...
# 2. Googlebot (Often bypasses WSJ blocks)
HEADERS_GOOGLEBOT = {
...
def parse_sitemap(content, site_name):
    ...
    try:
        ...
        for child in root:
            if 'url' in child.tag.lower():
                ...
                for sub in child:
                    ...
                    if 'lastmod' in tag_name or 'publication_date' in tag_name:
        ...
        # DEBUG: Print the first 100 chars to see if it's an HTML block page
        try:
            print(f"   [DEBUG] Content received: {content[:100].decode('utf-8', errors='ignore')}...")
        except:
            pass
...
for site_name, sitemap_url in SITEMAPS.items():
    ...
    # CHOOSE IDENTITY: Use Googlebot for WSJ, Chrome for others
    ...
    try:
        ...
        # requests automatically decodes gzip if headers allow, but sometimes .gz files are raw
        content = response.content
        
        # Check for gzip magic number (1f 8b) to avoid "Warning" spam
        if content.startswith(b'\x1f\x8b'):
            try:
                content = gzip.decompress(content)
            except OSError:
                pass # It wasn't valid gzip, stick with original content
